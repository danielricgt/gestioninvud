...
export const GET_BIENES = option => {
    ...
    switch (option) {
        ...
        case 8:
            filters = `, $fk_dependencia: Int!`
            where = `, where: {_and: [{fk_estado: {_in: [1, 2]}}, {fk_dependencia: {_eq: $fk_dependencia}}]}`
            break;
...
export const GET_NOTIFICACIONES = gql`
    query notificaciones($fk_usuario: Int!) {
        notificaciones(order_by: { id: desc }, where: { _and: [{ fk_estado: { _nin: [2,3, 9] } }, { fk_usuario_destino: { _eq: $fk_usuario } }] }) {
            id
            usuario_destino {
                id
                nombres
...
export const GET_NOTIFICACIONES_ORIGEN = gql`
    query notificaciones($fk_usuario: Int!) {
        notificaciones(order_by: { id: desc }, where: { _and: [{ fk_estado: { _in: [3, 9] } }, { fk_usuario_origen: { _eq: $fk_usuario } }] }) {
            id
            usuario_destino {
                id
                nombres
...
export const GET_NOTIFICACIONES_APROBADOR = gql`
    query notificaciones($fk_usuario: Int!) {
        notificaciones(order_by: { id: desc }, where: { _and: [{ fk_estado: { _nin: [2,3, 9] } }, { fk_usuario_aprobador: { _eq: $fk_usuario } }] }) {
            id
            usuario_destino {
                id
                nombres
